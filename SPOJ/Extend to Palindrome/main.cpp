...
void solve() {
    ...
    while (cin >> s1) {
        ...
        int x = fail[s2.size() - 1];

        cout << s1;
        for (int i = (int) s1.size() - x - 1; i >= 0; --i)
            fix += s1[i];
        cout << fix << '\n';
    }
}
...
/*
const int N = 1e6 + 10, mod = 1e9 + 7, b = 53, b2 = 59;
...
void solve() {
    ...
    while (cin >> s) {
        ...
        if (h1->get_hash(1, (int) s.size()) == h2->get_hash(1, (int) s.size())) {
            cout << s << '\n';
            continue;
        }
        ...
        for (int i = 1; i <= (int) s.size(); ++i) {
            if (h1->get_hash(i, (int) s.size()) == h2->get_hash(1, (int) s.size() - i + 1)) {
                last = i - 1;
                break;
            }
        }
